...
describe('when there is initially two users at db', () => {
  beforeEach(async () => {
    ...
    const user2 = new User({ username: 'niosni', passwordHash: passwordHash2 })
  ...
  test('creation fails when no username or password is given', async () => {
    const usersAtStart = await helper.usersInDb()
    ...
    expect(result2.body.error).toContain('username: Path `username` is required.')
